...
small battery-powered drone. Although they do not provide as much computational power as their larger, more power-hungry counterparts, it is our hope that I may successfully run neural-network-based target tracking software on these modules, in flight. 
...
Research on monocular, vision-based target tracking for UAVs appears sparse, despite continuing interest and advances in vision-based target tracking. For small, rotary wing drones I know of only a few previous attempts to automate target tracking \cite{basit2015joint,lin2012robust,boudjit2015detection}. In all of these works, the tracking and control logic is handled remotely and communicated to the drone via wifi. I wish, instead, to perform
target-tracking and control logic onboard the UAV, mainly to eliminate the dependency on communication with a remote entity, but also to reduce latency in video processing and control decisions. Recent advances in computer hardware, notably the availability of low-power GPGPUs, allow for a computer to be carried on-board a small drone. Further, these small, highly parallel modules do not draw much power, so they do not require drones to carry
larger batteries, and can compute large amounts of data in parallel, which means that neural networks can run quickly on them.
...
use OTB to evaluate their results, and using it again here provides a measure of continuity with their research.
