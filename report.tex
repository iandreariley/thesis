...
The second approach would be to keep the same basic architecture (i.e. kernel size and depth) while reducing the number of total parameters by reducing the number of kernels in each layer.
...
Another obvious approach would be to simply increase the available memory on the device. As a research goal this is not of much interest, but it would serve to validate whether having the whole network in memory significantly increases performance and, if so, by how much.
