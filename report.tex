...
In this research I implement a small onboard tracking module intended for a hexcopter UAV. I implement two computer-vision-based tracking algorithms for the module and compare their performance. One is a so-called "feature-based" tracker, the other uses a convolutional neural network. The results show that the neural-network-based method out-performs the feature-based method, but runs too slowly for a real-time application.
...
The first two lines of the algorithm are fairly trivial. OpenCV provides an API for reading images from files, which makes reading from a camera simple, since UNIX systems treat all devices as files. OpenCV also provides an API for drawing shapes as well as for tracking mouse movement, which the code in \cite{Nebehay2015CVPR} uses to make a simple GUI for the user to draw a bounding box around the target, and which I use in the tracking code.
